@php use App\Enums\Models\Permission\Can; @endphp
<div x-data="{ activeParent: null }" class="space-y-6">
    <!-- User Information Section -->
    <x-card class="bg-white dark:bg-gray-700 shadow-md">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between">
            <div class="mb-4 md:mb-0">
                <h2 class="text-2xl font-bold text-gray-800 dark:text-white">{{ __('Manage Permissions') }}: {{ $model->name }}</h2>
                @if(property_exists('email', $model))
                    <p class="text-gray-600 dark:text-gray-300">{{ __('Email') }}: {{ $model->email }}</p>
                @endif
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('Last updated') }}: {{ $model->updated_at->localFormat() }}</p>
            </div>
    ...
    <!-- Help Information -->
    <div class="bg-blue-50 dark:bg-blue-900 border-l-4 border-blue-500 p-4 mb-4 rounded-md">
        <div class="flex">
    ...
    <x-card class="bg-white dark:bg-gray-700 shadow-md">
        <div class="flex justify-between items-center">
            <div>
                <h3 class="text-lg font-medium text-gray-700 dark:text-white">{{ __('Permission Summary') }}</h3>
                <p class="text-sm text-gray-600 dark:text-gray-300">{{ __('Total permissions granted') }}: {{ $model->permissions->count() }}</p>
            </div>
        </div>
    </x-card>
    ...
    <div class="flex flex-col md:flex-row gap-6">
        ...
        <div class="w-full md:w-1/4 bg-white dark:bg-gray-700 shadow-md rounded-lg overflow-hidden">
            <div class="p-4 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-600">
                <h3 class="font-medium text-gray-700 dark:text-white">{{ __('Permission Groups') }}</h3>
            </div>
            <div class="divide-y divide-gray-200 dark:divide-gray-600">
                @foreach($permissions as $parentName => $children)
                    <div
                        ...
                        :class="{'bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-300': activeParent === '{{ $parentName }}'}"
                    >
                        <div class="font-medium dark:text-gray-200">{{ mb_ucfirst($parentName) }}</div>
                        <svg
        ...
        <div class="w-full md:w-3/4">
                                    ...
                                    <div class="bg-gray-50 dark:bg-gray-600 p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-500 transition-colors">
                                        <div class="flex items-center">
                                            ...
                                            <label for="permission-{{ $action['value'] }}" class="ml-2 block">
                                                <span class="font-medium text-gray-700 dark:text-white">{{ mb_ucfirst($action['name']) }}</span>
            ...
            <div x-show="activeParent === null" class="bg-white dark:bg-gray-700 shadow-md rounded-lg p-8 text-center">
                <svg class="mx-auto h-12 w-12 text-gray-400 dark:text-gray-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" />
                </svg>
                <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('No permission group selected') }}</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">{{ __('Select a permission group from the sidebar to view and manage permissions.') }}</p>
            </div>
        </div>
    </div>
</div>
