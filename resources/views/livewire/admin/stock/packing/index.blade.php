@php use App\Models\Packing; @endphp
<div>
    <x-card>
        <x-slot:header>
        ...
        <x-table :headers="$this->headers" :$sort :rows="$this->rows" paginate simple-pagination filter loading :quantity="[2, 5, 15, 25]">
            ...
            @interact('column_action', $row)
                <div class="flex gap-1 justify-end">
                    ...
                    @endcan
                    @can('delete', $row)
                        <livewire:admin.stock.packing.delete :packing="$row" :key="uniqid('', true)" @deleted="$refresh" />
                    @endcan
                </div>
            @endinteract
        </x-table>
    ...
    <livewire:admin.stock.packing.update @updated="$refresh" />
</div>
